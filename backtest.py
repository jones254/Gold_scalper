...
For each trading day T in the backtest window:
    ...
    3. Measure the realised forward return of the target asset over the holding window.

We track:
    - Hit rate (% of signals whose forward return agreed with direction)
    - Mean / median / std return per bucket
    - Equity curve of a long-flat strategy: long when the model says
...
class BacktestConfig:
    ...
    holding_period: int = 5           # forward window in trading bars
...
def run_backtest(
    ...
    # ----- 3. Forward return of target asset --------------------------------
    target_close = data["gold"]["Close"].reindex(df.index)
