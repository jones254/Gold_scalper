...
This is the Python port of the Pine Script logic, adapted for gold's
macro drivers: DXY, yields (IEF), silver, equities, EUR/USD, VIX.
...
def market_regime(per_market: Dict[str, pd.Series]) -> pd.Series:
    ...
    DXY is already sign-inverted in `per_market` (positive = risk-off for gold),
    so we use the raw scores directly.
    """
    dxy = per_market["dxy"]
    ief = per_market["ief"]
...
def flow_meter(
    ...
    Higher = more gold-bullish institutional environment.
    """
    composite_n = (composite + 100.0) / 2.0

    # DXY: lower is better for gold
    dxy_v = dxy_close.clip(lower=0.0).fillna(100.0)
    dxy_n = ((1.0 - (dxy_v - 90.0) / 20.0).clip(0.0, 1.0)) * 100.0

    # VIX: higher is better for gold (safe-haven demand)
    vix_v = vix_close.clip(lower=0.0).fillna(15.0)
    vix_n = (vix_v / 40.0).clip(0.0, 1.0) * 100.0

    # IEF: higher bond prices = lower yields = better for gold
    # Normalize relative to a 200-bar moving average
    ief_v = ief_close.fillna(method="ffill")
