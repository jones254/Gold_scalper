...
with tab_live:
    ...
    # ----- Top strip: macro context (composite + flow, compact) -------------
    # The composite is intentionally demoted to a small pill.  The primary
    # action signal lives in the three forecast cards below — that's what
    # drives the entry setup, since the composite is most often Neutral.
    last_flow = float(flow.loc[latest])
    flow_delta = last_flow - 50.0
    ...
    with col_b:
        st.metric("Institutional Flow", f"{last_flow:.0f}/100",
                  delta=f"{flow_delta:+.0f} vs neutral")
    with col_c:
    ...
    # ----- Primary forecast cards (clickable) ------------------------------
    # Three cards, one per horizon.  Each card shows the live label, score,
    # confidence, and humanized duration.  Click a card to make it the
    # active horizon for the entry section below.  This is the primary
    # action surface — the composite is just background context.
    st.markdown("##### 🎯 Forecast horizons — pick one for the entry setup")

    # Initialize selection if first render
    if "active_horizon" not in st.session_state:
        st.session_state.active_horizon = "medium"

    # Refresh the default if the user hasn't actively picked (e.g. after
    # sidebar change).  We keep whichever horizon is most "interesting"
    # (highest absolute score) as the suggested pick.
    ...
    for col, hname in zip(card_cols, ("short", "medium", "long")):
        ...
        ring = "outline: 3px solid #1f77b4;" if is_active else "outline: 1px solid #ddd;"
        ...
        with col:
            ...
            # Click button below the card to make it active
    ...
    if _atr is not None and "de40" in data and len(data["de40"]) > 30:
        ...
        with hz_cols[0]:
            # Horizon is now selected by clicking one of the forecast cards
            # at the top of the tab.  Show it as a read-only badge here.
            st.markdown(
                f"""<div style="background:#1f77b41a;border-left:4px solid #1f77b4;
                            padding:8px 12px;border-radius:6px">
                      <div style="font-size:0.7rem;color:#666">ACTIVE HORIZON</div>
                      <div style="font-size:1.1rem;font-weight:700;color:#1f77b4">{horizon.upper()}</div>
    ...
    # ----- Forecasts panel (compact, contextual) ---------------------------
    # Show the three horizon scores side by side, but small — the primary
    # forecast display is the three clickable cards at the top of the tab.
    st.caption("Forecast scores for the three horizons (current bar):")
    fc1, fc2, fc3 = st.columns(3)
    for col, hname in zip((fc1, fc2, fc3), ("short", "medium", "long")):
        with col:
            sc   = float(fcasts[hname].loc[latest])
            lbl  = str(f_labels[hname].loc[latest])
            conf = float(f_conf[hname].loc[latest])
            c    = SIGNAL_COLOURS.get(lbl, "#888")
            is_active = (horizon == hname)
            bold = "font-weight:700;" if is_active else ""
            st.markdown(
                f"""<div style="padding:4px 6px">
                      <span style="color:#666;font-size:0.78rem">{hname.upper()}</span>
                      &nbsp; <span style="color:{c};{bold}font-size:0.95rem">{lbl}</span>
                      &nbsp; <span style="color:#888;font-size:0.78rem">{sc:+.1f} · {conf:.0f}%</span>
                    </div>""",
                unsafe_allow_html=True,
            )
