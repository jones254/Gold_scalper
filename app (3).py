...
# Defensive: ensure the new multi-timeframe config fields exist
required_attrs = ["periods_short", "periods_medium", "periods_long"]
missing = [a for a in required_attrs if not hasattr(config, a)]
if missing:
    st.error(f"Config is missing fields: {missing}. Please update config.py to the latest version with multi-timeframe composite horizons.")
    st.stop()
